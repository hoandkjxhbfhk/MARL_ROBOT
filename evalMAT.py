...
class MATAgent:
    def __init__(self, agent_params):
        ...
        # Sửa: lấy đúng shape cent_obs_space từ env (không dùng observation_space[0] nếu centralized khác)
        cent_obs_space = observation_space[0]
        if hasattr(temp_env_for_spaces, 'share_observation_space'):
            cent_obs_space = temp_env_for_spaces.share_observation_space[0]
        ...
        self.policy = TransformerPolicy(self.args,
                             ...
                             cent_obs_space=cent_obs_space, # Tạm thời dùng obs_space cho cent_obs_space nếu use_centralized_V=False, cần kiểm tra lại
    ...
    def get_actions(self, obs, share_obs, available_actions, deterministic=True):
        ...
        # Debug: in ra shape của từng phần tử trong share_obs
        #print("[DEBUG] share_obs type:", type(share_obs))
        if isinstance(share_obs, list):
            #for i, s in enumerate(share_obs):
            #    print(f"[DEBUG] share_obs[{i}] shape: {np.array(s).shape}")
            share_obs_np = np.stack([np.array(s) for s in share_obs], axis=0)
        else:
            share_obs_np = np.array(share_obs)
        #print("[DEBUG] share_obs_np shape:", share_obs_np.shape)

        # Đảm bảo shape là (n_agents, obs_dim)
        if share_obs_np.shape[0] != self.args.n_agents:
            raise ValueError(f"share_obs first dim {share_obs_np.shape[0]} != n_agents {self.args.n_agents}")
        obs_dim = share_obs_np.shape[1]
        share_obs_flat = share_obs_np.reshape(1, self.args.n_agents, -1)  # (1, n_agents, n_agents*obs_dim)
        print("[DEBUG] share_obs_flat shape:", share_obs_flat.shape)

        masks = np.ones((1, self.args.n_agents, 1), dtype=np.float32)

        result = self.policy.get_actions(
            cent_obs=share_obs_flat,
            obs=obs_reshaped,
        ...
        # MAT trả về: value, actions, action_log_prob, rnn_states_actor, rnn_states_critic_
        value, actions, action_log_prob, rnn_states_actor, rnn_states_critic_ = result
        self.rnn_states = _t2n(rnn_states_actor)
        self.rnn_states_critic = _t2n(rnn_states_critic_)
        # Fix: robustly flatten actions to (n_agents,)
        return _t2n(actions).reshape(-1)
...
def run_eval(agent_type, agent_params, num_episodes, env_config):
    ...
    for ep in range(num_episodes):
        ...
        if agent_type == "mat":
            ...
            n_delivered = env.get_total_delivered_packages() # Sử dụng hàm mới trong DeliveryMATEnv
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mat_model_dir", type=str, default="Multi-Agent-Transformer/mat/scripts/results/delivery/mat/map2_5agents_50pkgs/check/run2/models/transformer_155.pt", 
