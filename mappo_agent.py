...
from mappo import convert_observation, generate_vector_features
from mappo import ActorNetwork as AgentNetwork
...
class Agents:
    def __init__(self, observation_shape, vector_obs_dim, max_time_steps, weights_path=None, device=None):
        ...
        self.model = AgentNetwork(observation_shape, vector_obs_dim, JOINT_ACTION_DIM).to(self.device) # Pass JOINT_ACTION_DIM
    ...
    def get_actions(self, state, deterministic=True):
        ...
        for i in range(self.n_robots):
            ...
            # Ensure n_robots is at least 1 for max_other_robots_to_observe
            max_other_robots_to_obs = max(0, self.n_robots - 1)
